...
def prepare(client: adbutils.AdbDevice):
    ...
    print(subprocess.run(f"adb -s {client.serial} push  {os.getcwd()}/ncnn/latest/benchncnn /data/local/tmp/",
...
def run(client: adbutils.AdbDevice):
    try:
        os.mkdir("adbLogs/cpuLogs/ncnn")
    except:
        pass
    # CPU
    log_filename = "adbLogs/ncnn/cpu.log"
    frequency_monitor = subprocess.Popen('adb shell "top -d 1"|grep benchncnn |grep -v grep >adbLogs/cpuLogs/ncnn/cpu.log',
                                             shell=True)
    logs = client.shell(
    ...
    frequency_monitor = subprocess.Popen('adb shell "top -d 1"|grep benchncnn |grep -v grep >adbLogs/cpuLogs/ncnn/gpu.log',
                                             shell=True)
    logs = client.shell(
